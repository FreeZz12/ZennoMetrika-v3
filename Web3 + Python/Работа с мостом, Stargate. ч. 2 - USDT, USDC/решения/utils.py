...
def get_tx_params(w3: Web3, to: str | ChecksumAddress | None = None, value: int | None = None,
...
def get_token_price_from_binance(token_symbol: str) -> float:
    ...
    data = response.json()
    return float(data['price']) if 'price' in data else 1
...
def approve(w3: Web3, token: Token, spender: str, amount: Amount, private_key: str) -> None:
    """
    ...
    if allowance < amount.wei:
        tx_params = token_contract.functions.approve(spender, amount.wei).build_transaction(get_tx_params(w3))
