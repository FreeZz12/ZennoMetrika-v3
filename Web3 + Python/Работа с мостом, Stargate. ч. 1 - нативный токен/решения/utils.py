...
def approve(w3: Web3, spender: str, amount_wei: int, private_key: str, token: Token | None = None) -> None:
    """
    Функция approve для токенов ERC20
    :param w3: объект web3
    :param token: объект токена
    :param spender: адрес контракта, которому нужно дать разрешение
    ...
    if token is None:
        return
    contract = w3.eth.contract(address=token.address, abi=get_abi('erc20'))
    allowance = contract.functions.allowance(w3.eth.default_account, spender).call()
    if allowance >= amount_wei:
        return
    spender = w3.to_checksum_address(spender)
    tx_params = contract.functions.approve(spender, amount_wei).build_transaction(get_tx_params(w3))
