...
from utils import get_tx_params, sing_send_tx, approve
...
def bridge(to_chain: Chain, amount: Amount, token: Token| None = None):
    ...
    # получаем объект с данными контрактов
    contracts_data = Contracts.get_contracts_data_by_name(f'stargate_pool_{token_name}')
    # получаем объект с данными контракта по конкретной сети
    contract_data = contracts_data[work_chain]
    # делаем approve, если это токен erc20
    approve(w3, contract_data.address, amount.wei, config.private_key, token)
    # создаем инстанс контракта
    contract = contract_data.get_contract(w3)
    # получаем комиссию
    fee = get_stargate_fee(to_chain, contract, amount.wei)
    # если НЕ нативный токен, то отправляем только комиссию
    value = fee[0] if token else amount.wei + fee[0]
...
def main():
    ...
    amount = Amount(0.5, 6)
    bridge(to_chain=Chains.ARBITRUM, amount=amount, token=token)
